...
from app.models.models import (
    TestAssign, Test, Employee, Quiz, QuizResult
)
...
def submit_test(
    ...
    if test.quiz_id:
        quiz = db.query(Quiz).filter(Quiz.id == test.quiz_id).first()
        if quiz:
            correct_answers = {q: v["correctAnswer"] for q, v in quiz.questions.items()}
    score = sum(
...
async def get_feedback_for_result(
    ...
    from app.Agents.FeedbackAgent.FeedbackAgent import generate_feedback
    import json
