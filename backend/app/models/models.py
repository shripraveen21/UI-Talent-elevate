...
class Test(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "(CASE WHEN quiz_id IS NOT NULL THEN 1 ELSE 0 END + CASE WHEN debug_test_id IS NOT NULL THEN 1 ELSE 0 END) = 1",
            name='chk_single_test_type'
        ),
    )
...
class BandLevelRule(Base):
    __tablename__ = 'band_level_rules'
    id = Column(Integer, primary_key=True)
    band = Column(Enum(BandType), nullable=False)
    allowed_level = Column(Enum(SkillLevel), nullable=False)
    __table_args__ = (
        UniqueConstraint('band', 'allowed_level', name='uniq_band_level'),
    )
