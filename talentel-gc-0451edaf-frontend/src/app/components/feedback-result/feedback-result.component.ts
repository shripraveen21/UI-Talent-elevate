...
export class FeedbackResultComponent {
      ...
      if (testId) {
        this.testListingService.getTestAttempts(testId).subscribe({
          next: (data: any) => {
            ...
            console.log(this.attemptedEmployees,"at")
          },
          error: (error) => {
    ...
    navigateToHandsOnResults(handsonId:number) {
    ...
    navigateToQuizResults(quiz_id:number) {
      this.router.navigate(["/results",this.selectedTestId])
    }
    
    navigateToDebugResults(debug_id:number) {
      console.log(debug_id)
      this.router.navigate(["/debug-results",debug_id])
    }
