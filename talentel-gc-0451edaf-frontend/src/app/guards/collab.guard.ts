...
export class CollabGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
    ...
    return this.permissionsService.getUserPermissions().pipe(
      map(permissions => {
        if (
          permissions.role === 'CapabilityLeader'
        ) {
          return true;
        }
        if (
          (permissions.role === 'ProductManager' ||
            permissions.role === 'DeliveryManager' ||
            permissions.role === 'DeliveryLeader') &&
          requiredPermission !== 'topics' &&
          requiredPermission !== 'collaborator'
        ) {
          return true;
        }
