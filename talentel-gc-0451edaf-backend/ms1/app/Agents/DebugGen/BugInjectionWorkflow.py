...
class BugDesignAgent:
    def __init__(self, model_client, project_dir, difficulty="intermediate"):
        self.agent = AssistantAgent(
            name="bug_designer",
            model_client=model_client,
            system_message=f"""
...
"""
    ...
    async def design_bugs(self, project_dir, candidates, topics, difficulty):
        termination = TextMentionTermination("TERMINATE")
        team = RoundRobinGroupChat([self.agent], termination_condition=termination)
        task = f"""
        ...
        For each candidate location, design a realistic, educational bug related to the topic and appropriate for the {difficulty} level.
        """
        result = await team.run(task=task)
        json_content = ""
        for message in result.messages:
...
# --- Main Workflow ---
async def bug_injection_workflow(model_client, unique_id, topics, difficulty):
    try:
        ...
        design_agent = BugDesignAgent(model_client, bugged_dir, difficulty)
        bug_plans = await design_agent.design_bugs(bugged_dir, candidates, topics, difficulty=difficulty)
...
if __name__ == "__main__":
    ...
    asyncio.run(bug_injection_workflow(
        model_client,
        "50983da2-44c9-4a30-bb88-cf6744fd7deb",
